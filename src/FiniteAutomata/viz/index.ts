...
export const vizualize = (name: string, machine: FiniteAutomata<any, any>) => {
  let viz = new Viz({ Module, render });

  let index = 0;

  const generateVizString = (machine: FiniteAutomata<any, any, any>) => {
    const machineIndexMapping = new Map<
      FiniteAutomata<any, any, any>,
      number
    >();
  ...
  viz
    ...
    .then((result) => {
      fs.writeFileSync(`./result/${name}.svg`, result);
    })
    .catch((error) => {
